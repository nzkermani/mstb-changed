function bin = nzkPeakMatching3Methods(path, file, ppm, binOfLengthOneOK,splitCorrection, method )
...
parfor i=1:(number_of_intervals+1)
    ...
    switch method
        case 'RG'
            setGlobalx(i);
            bin{i}  = recursiveGap( interval(i).ions,ppm/1e6 ,1 );
        case 'RM'
            setGlobalx(i);
            bin{i}  = recursiveMorris( interval(i).ions, ppm/1e6 ,1 );
        case 'nn'
...
if(isequal(method, 'nn'))
  ...
  [bin]= convertToBin(dims, bin,sorted_mz_indx,sorted_mz_value,TempSp,'restrict');
end
