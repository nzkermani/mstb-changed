...
%thing and calculate ppm differences between neighbouring data points.

if nargin == 1
    fig = false;
end

% Define a ppm threshold at which point we decide points are not contiguous
thresh = 0.5;
...
for n = 1:numO
    ...
    for s = 1:numS
        ...
        if fig && any(plotInd == s)           
            ...
            %t%mpY = polyval(loopFit,tmpX);
            
            % Add to the figure
            %plot(tmpAx,tmpX,tmpY,'--','Color',cols(n,:));
        end 
    end
...
if fig
    idx2 = randperm(numel(idx),numel(idx)-200);
    idx(idx2) = false;
